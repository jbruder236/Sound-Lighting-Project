...
from spectrum import BANDS, HUES
...
class BandChart(Static):
    ...
    def render(self):
        ...
        if width >= 60:
            ...
            bar = max(2, width - 24)
            for name, hz, level, color in zip(BANDS, RANGES, self.levels, COLORS):
...
class SpectrumPane(Vertical):
    ...
    def show_status(self, data):
        ...
        state = ('Offline · waiting for Pi' if stale else
                 'Frequency → hue · audio → glow' if data.get('spectrum_active') else
