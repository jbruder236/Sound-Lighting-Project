...
# 7/20 Patch Notes:
# -Changed brightSet multiplier from 2.55 to 1.5
# -Introduced maxPeak var to terminal printout for debugging
# -Removed curColor check
# -Added rainbow coloring
# -Added iteration timing w/ terminal printout
...
def soundLight(strip, wait_ms=20):
    maxDelta = 5
    lastPeak = 0
    maxPeak = 0 #for use in detecting peak value at which strip flickers
    totalTime = 0
    for i in range(256*20):
        ...
        #print("i:%04d  peak:%03d   Delta:%02d   maxPeak:%03d"% (i, peak, peakDelta, maxPeak))
        fTime = time.time()
